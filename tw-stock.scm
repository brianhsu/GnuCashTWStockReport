...
(define-module (gnucash reports standards tw-stock))
...
(define tax-rate-etf 1000)
(define tax-rate 3000)

(define reportname (N_ "台灣股市投資組合"))

(define pagename-fee (N_ "Fee"))
...
(define optname-fee-discount-percentage (N_ "手續費折扣 % 數"))
(define optname-fee-minimal-charge (N_ "整股手續費低消（元）"))
(define optname-fee-minimal-charge-for-odd-share (N_ "零股手續費低消（元）"))
...
(define (calculate-tax ticker-symbol current-value)
  (let* ((first-two-symbol (substring ticker-symbol 0 2)) 
	 (is-etf (string=? first-two-symbol "00")))
    (round (if is-etf (/ current-value tax-rate-etf) (/ current-value tax-rate)))))

(define (calculate-entire-shares-fee minimal-charge discount-percentage shares unit-price)
  (let* ((total-shares (* 1000 shares))
	 (total-price (* total-shares unit-price))
	 (official-fee (* total-price 0.001425))
	 (discount-rate (/ (- 100 discount-percentage) 100))
	 (discounted-fee (* official-fee discount-rate)))
    (if (< discounted-fee minimal-charge) minimal-charge (round discounted-fee))
))

(define (calculate-odd-shares-fee minimal-charge discount-percentage shares unit-price)
  (let* ((total-price (* shares unit-price))
	 (official-fee (* total-price 0.001425))
	 (discount-rate (/ (- 100 discount-percentage) 100))
	 (discounted-fee (* official-fee discount-rate)))
    (if (< discounted-fee minimal-charge) minimal-charge (round discounted-fee))
...
(define (calculate-fee report-obj shares current-value)
  (define (get-option section name)
    (gnc-optiondb-lookup-value (gnc:report-options report-obj) section name))

  (let* ((entire-share-minimal-charge (get-option pagename-fee optname-fee-minimal-charge))
	 (odd-share-minimal-charge (get-option pagename-fee optname-fee-minimal-charge-for-odd-share))
	 (discount-percentage (get-option pagename-fee optname-fee-discount-percentage))
	 (entire-shares (quotient shares 1000))
	 (odd-shares (remainder shares 1000))
	 (unit-price (/ current-value shares))
	 (entire-shares-fee 
	   (if (= 0 entire-shares) 
	     0 
	     (calculate-entire-shares-fee entire-share-minimal-charge discount-percentage entire-shares unit-price)))
	 (odd-shares-fee 
	   (if (= 0 odd-shares) 
	     0 
	     (calculate-odd-shares-fee odd-share-minimal-charge discount-percentage odd-shares unit-price))))
    (inexact->exact (+ entire-shares-fee odd-shares-fee))))

(define (options-generator)
  (let* ((options (gnc-new-optiondb)))

    (gnc-register-number-range-option options
      pagename-fee optname-fee-discount-percentage
      "a" (N_ "券商的手續費折扣數百分比，例如打 95 折請填 5，打 2 折填 80")
      90  ;; default
      0   ;; lower bound
      100 ;; upper bound
      1  ;; step size
    ...
    (gnc-register-number-range-option options
      pagename-fee optname-fee-minimal-charge
      "b" (N_ "賣出一張以上股票的券商手續費低消（元）")
      20    ;; default
      0     ;; lower bound
      10000 ;; upper bound
      1     ;; step size
    ...
    (gnc-register-number-range-option options
      pagename-fee optname-fee-minimal-charge-for-odd-share
      "c" (N_ "賣零股時的券商手續費低消（元）")
      1     ;; default
      0     ;; lower bound
      10000 ;; upper bound
...
(define (table-add-stock-rows table accounts to-date
   ...
   (let ((share-print-info
    ...
    (define (table-add-stock-rows-internal accounts odd-row?)
      (if (null? accounts) total-value
		  ...
		  (current-value (gnc:gnc-monetary-amount (my-exchange-fn value currency)))
		  (utax-value (calculate-tax ticker-symbol current-value))
                  (utax (gnc:make-gnc-monetary currency utax-value))
		  (ufee-value (calculate-fee report-obj units current-value))
		  (ufee (gnc:make-gnc-monetary currency ufee-value))
		  ...
		  (all-basis (sum-basis basis-list (gnc-commodity-get-fraction currency)))
		  (all-cost (gnc-numeric-add (+ utax-value ufee-value) all-basis currency-frac GNC-RND-ROUND))
              ...
              (total-utax 'add (gnc:gnc-monetary-commodity utax) (gnc:gnc-monetary-amount utax))
              (total-ufee 'add (gnc:gnc-monetary-commodity ufee) (gnc:gnc-monetary-amount ufee))
              ...
              (append! activecols (list (if use-txn (if pricing-txn "*" "**") " ")
                                        ...
                                        (gnc:make-html-table-header-cell/markup "number-cell" utax)
                                        (gnc:make-html-table-header-cell/markup "number-cell" ufee)
  ...
  (let ((to-date     (gnc:time64-end-day-time
        ...
        (total-utax (gnc:make-commodity-collector))    ;; unrealized tax
        (total-ufee (gnc:make-commodity-collector))    ;; unrealized fee
    ...
    (if (not (null? accounts))
        ...
        (let* ((exchange-fn (gnc:case-exchange-fn price-source currency to-date))
               ...
               (sum-total-utax (gnc-numeric-zero))
               (sum-total-ufee (gnc-numeric-zero))
          ...
          (catch 'div/0
            (lambda ()
              (table-add-stock-rows
               ...
               total-income total-gain total-utax total-ufee total-ugain total-brokerage))
            (lambda (k reason)
          ...
          (set! sum-total-utax (gnc:sum-collector-commodity total-utax currency exchange-fn))
          (set! sum-total-ufee (gnc:sum-collector-commodity total-ufee currency exchange-fn))
          ...
          (append! totalscols (list
                               ...
                               (gnc:make-html-table-cell/markup
                                "total-number-cell" sum-total-utax)
                               (gnc:make-html-table-cell/markup
                                "total-number-cell" sum-total-ufee)
                               (gnc:make-html-table-cell/markup
        ...
        (gnc:html-document-add-object! 
         document 
         (gnc:make-html-text 
          (gnc:html-markup-p (G_ "<ol>
				 <li>未實現獲利（虧損）已扣除預估賣出時之交易稅、手續費</li>
				 <li>預估證交稅計算方式：股數*市價*稅率</li>
				 <li>預估手續費計算方式為以下兩者相加：
				   <ul>
				     <li>優先以整張股票計算，套用折扣後若低於整股手續費低消以低消計</li>
				     <li>剩餘零股套用折扣後，若低於零股手續費低消，以低消計</li>
				   </ul>
				 </li>
				 </ol>"))))
...
(gnc:define-report
 'version 1
 'report-guid "1badf88557744e15b858447bfd2dc90a"
 'name reportname
